...
public class special_executor
{
    ...
    public void GetPreBuiltQueries()
    {
        try
        {
            ...
            string query = $"select name,query from pre_built_queries where control < 10";
    ...
    private string PickFrom(string special_request)
    {
        while (special_request.ToLower().Contains("<pick_from_") && special_request.Contains(">"))
        {
            ...
            string pick_from = special_request.Substring(special_request.IndexOf("<pick_from_") + 11, special_request.IndexOf(">") - (special_request.IndexOf("<pick_from_") + 11));
            try
            {
